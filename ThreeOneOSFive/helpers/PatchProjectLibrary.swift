...
enum PatchProjectLibrary {
    ...
    static func installBundledPackagesIfNeeded(
        ...
        let extensions = ["3105", "shw", "cryptroic"]
        var bundledURLs: [URL] = []
        var seen = Set<String>()
        for ext in extensions {
            let nested = bundle.urls(forResourcesWithExtension: ext, subdirectory: "Patches") ?? []
            let flattened = bundle.urls(forResourcesWithExtension: ext, subdirectory: nil) ?? []
            for u in (nested + flattened) {
                if seen.insert(u.standardizedFileURL.path).inserted {
                    bundledURLs.append(u)
                }
            }
        }
    ...
    static func load(fileManager: FileManager = .default) -> [PatchLibraryItem] {
        ...
        for url in urls where ["3105", "cryptroic"].contains(url.pathExtension.lowercased()) {
            do {
    ...
    static func save(
        ...
        if let existingURL {
            ...
            var candidate = root.appendingPathComponent(baseName).appendingPathExtension("shw")
            var suffix = 2
            while fileManager.fileExists(atPath: candidate.path) {
                candidate = root.appendingPathComponent("\(baseName)-\(suffix)").appendingPathExtension("shw")
                suffix += 1
            }
